...
```bash
lambda-pythonSetpFunction$ sam build --use-container
```
...
```bash
lambda-pythonSetpFunction$ sam logs -n StockCheckerFunction --stack-name "lambda-pythonsetpfunction" --tail
```
...
```bash
lambda-pythonSetpFunction$ pip install -r tests/requirements.txt --user
# unit test
lambda-pythonSetpFunction$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
lambda-pythonSetpFunction$ AWS_SAM_STACK_NAME="lambda-pythonsetpfunction" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "lambda-pythonsetpfunction"
```
